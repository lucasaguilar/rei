...
export async function chunkRepoMap(workspacePath: string): Promise<RepoMapChunk[]> {
  ...
  try {
    ...
    return chunkRepoMapString(content);
  } catch (err) {
...
/**
 * Parses a string containing repo map declarations into individual chunks.
 */
export function chunkRepoMapString(content: string): RepoMapChunk[] {
