...
export class DuckDuckGoLiteClient implements WebSearchClient {
      ...
      const req = https.request(
        ...
        (res) => {
          if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
            reject(new Error(`DDG strict redirect detected (${res.statusCode})`));
            return;
          }
  ...
  private parseResults(html: string): SearchResult[] {
    ...
    for (const chunk of docSplit) {
      // Find Title and URL
      const urlMatch = chunk.match(/class="result-url"\s+href="([^"]+)">([^<]+)<\/a>/i);
      if (urlMatch) {
      ...
      if (snippetMatch && currentUrl) {
         ...
         rawSnippet = rawSnippet.replace(/<[^>]+>/g, "").replace(/&[^;]+;/g, " ").replace(/\s+/g, " ").trim();
         ...
         if (realUrl.startsWith("//duckduckgo.com/l/?") || realUrl.startsWith("/l/?")) {
    ...
    if (results.length === 0) {
      ...
      while ((match = anchorRegex.exec(html)) !== null) {
        let realUrl = match[1];
        if (realUrl.startsWith("//duckduckgo.com") || realUrl.startsWith("/l/?")) {
