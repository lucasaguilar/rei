 ...
 */
/**
 * Renders an MCP tool's JSON-Schema input as a compact one-line `args:` hint so the
 * model knows parameter names, types, required-ness and enum values on the XML path
 * (ask/planning), where the structured `tools` API — and thus the schema — is NOT
 * otherwise available. Without this the model guesses argument names and loops (e.g.
 * calling github/pull_request_read with `issue_number` instead of the required
 * `pullNumber` + `method`). Returns "" when the tool declares no parameters.
 */
function formatMcpToolArgs(inputSchema: unknown): string {
  ...
  const parts = Object.entries(props).map(([name, raw]) => {
    ...
    const hint = Array.isArray(p.enum)
...
export function formatMcpToolsForPrompt(mcpTools: McpTool[]): string {
  ...
  const lines = mcpTools.map((tool) => {
    const args = formatMcpToolArgs(tool.inputSchema);
    return args
  ...
  return [
    ...
    "Each tool lists its `args:` — `*` marks required params, `{a|b}` shows the allowed values. Use these exact names.",
    "",
