...
export class InputHandler {
  public static async submitInput(ctx: InputHandlerContext): Promise<void> {
    ...
    if (palette.length > 0 && activePalette.kind === "command") {
      ...
      actions.resetInput();
      state.paletteClosed = true;
      const wasCommand = await InputHandler.handleCommand(
        selected.command,
        ctx,
      );
      actions.draw();
  ...
  private static async handleCommand(
    ...
    if (trimmed === "/confirm" || trimmed === "/confirm --dry-run") {
      ...
      try {
        ...
        // Run a full compile check after applying patches (passive feedback, no auto-fix)
        if (!dryRun && result.success) {
          state.activeStatus = "producing_response";
          actions.draw();
          try {
            const compileResult = await runCompileCheck(ctx.workspacePath);
            for (const line of formatCompileResult(compileResult)) {
              actions.pushTranscript(line);
            }
          } catch (compileErr: unknown) {
            // NOTE: Compile check is best-effort — don't crash the UI if ts-morph fails
            actions.pushTranscript(
        ...
        // Auto-save session after the turn
        saveSession(
          ctx.workspacePath,
          session.messages,
          session.mode,
  ...
  private static async handleUserTurn(
    trimmed: string,
    ctx: InputHandlerContext,
  ): Promise<void> {
