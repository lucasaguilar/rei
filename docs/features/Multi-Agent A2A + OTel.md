...
- ▲ **IP-3 Step + Orchestration spans** — `withStepSpan` at each loop head; **and** `withStageSpan` /
  ...
  **Agent-mode step spans now done (v8):** `startStepSpan` wraps each iteration of all three
  agent-mode loops (function-calling, XML search-replace, whole-file) as well as the ask/planning
  loop, so `step-N` nests under `rei.turn` in every mode. Non-streaming + orchestration spans still deferred.
...
- ▲ **IP-5 Tool span + `delegate_to_agent`** — **implemented (tool spans).** Every tool/command
  is traced as a `tool.<name>` span (Laminar `spanType: "TOOL"`, args as input) wrapped at the
  deepest chokepoint unique to each tool, so each logical call is one span with no double-wrap:
  `run_command` at `executeCommand`, MCP at `McpRegistry.dispatch`, `weather`/`search` in
  `dispatchXmlToolCall`, and the agent-native built-ins (`read_files`, `edit_file`, `rewrite_file`,
  `create_file`, `search_tools`, `use_skill`) via `startToolSpan` in the function-calling loop.
  `delegate_to_agent` + the unified `dispatchBuiltinTool` still pending (Wave 2 A2A work).
...
| **v8** | 2026-06-18 | **IP-5 tool spans implemented + IP-3 agent-mode step spans.** `withToolSpan` now sets Laminar `spanType: "TOOL"` with args as input (mirrors `llm-call`'s `"LLM"`); added `startToolSpan` (global-active start/end variant for state-mutating dispatch). Every tool/command is traced exactly once at the chokepoint unique to it — `run_command`→`executeCommand`, MCP→`McpRegistry.dispatch`, `weather`/`search`→`dispatchXmlToolCall`, agent-native built-ins→`startToolSpan` in the function-calling loop — covering all four dispatch paths with no double-wrap. `startStepSpan` extended to the three agent-mode loops (`generator-tools.ts` function-calling, `generator.ts` search-replace + whole-file), completing IP-3 for streaming agent mode; `tsc`+355 tests green. Non-streaming step spans, `delegate_to_agent`, unified `dispatchBuiltinTool`, and orchestration spans remain deferred. |
