...
async function runDiagnostic() {
  ...
  const testQuery = "How is the chat UI state managed and what variables does it have?";
  ...
  context.relevantFiles.forEach(f => {
    console.log(`  - ${f.path} (Score: ${f.score}) | Bytes injected: ${f.preview.length}`);
  });
  ...
  if (context.ragNodeSnippets && context.ragNodeSnippets.length > 0) {
    ...
    context.ragNodeSnippets.forEach(s => {
      console.log(`  - ${s.nodeName} (${s.filePath}) | Lines: ${s.startLine}-${s.endLine}`);
    });
  }
}
