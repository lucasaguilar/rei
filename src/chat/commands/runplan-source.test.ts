...
describe("precedence: the active plan owns the decision", () => {
  it("beats a session message that merely quotes another plan — the original bug", () => {
  ...
  it("saving a plan makes it the one that runs", () => {
    ...
    const r = run("/runplan stage 2", []);
    expect(r.success).toBe(true);
    expect(r.response).toContain('active plan "saved"');
  });

  it("falls back to the session when no plan is active, and says so", () => {
    const r = run("/runplan stage 1", planMsg(PLAN));
    expect(r.response).toContain("this session (no active plan set)");
  });

  it("warns when the active plan points at a file that is gone", () => {
    setActivePlan(ws, "deleted");
    const r = run("/runplan stage 1", planMsg(PLAN));
    expect(r.response).toMatch(/active plan "deleted" is set but .* is missing/);
  });

  it("picks up an edit to the active plan file — it holds a name, not a copy", () => {
    savePlanToFile(ws, "live", PLAN);
    setActivePlan(ws, "live");
    savePlanToFile(ws, "live", PLAN + "\n## Stage 3: added later\nFiles to modify: src/c.ts\n");
    const r = run("/runplan stage 3", []);
    expect(r.success).toBe(true);
...
describe("stage targets", () => {
  it("never targets REI's own artifacts — a plan that names its own path must not be edited", () => {
    ...
    const r = run("/runplan stage 2", planMsg(plan)) as unknown as {
  ...
  it("still passes ordinary markdown targets through", () => {
    const plan = "# Plan\n\n## Stage 1: Document it\nFiles to modify: docs/guide.md\n";
    const r = run("/runplan stage 1", planMsg(plan)) as unknown as {
      autoExecute?: { prompt: string };
    };
...
describe("/runplan names its source", () => {
  it("reports the session as the source, with the stage numbers it found", () => {
    const r = run("/runplan stage 1", planMsg(PLAN));
  ...
  it("reports the persisted fallback when the session has no plan", () => {
    saveCurrentPlanContent(ws, PLAN);
    const r = run("/runplan stage 1", []);
    expect(r.response).toContain(".rei/current-plan-content.md");
    expect(r.response).toContain("no plan in this session");
  });

  it("shows the source on a missing stage, so the wrong plan is obvious", () => {
    const r = run("/runplan stage 7", planMsg(PLAN));
  ...
  it("explains the heading format when a bold-only plan is on screen but unparseable", () => {
  ...
  it("prefers the newest session plan — the behaviour that surprises, now stated out loud", () => {
    const recap = "Here is a recap:\n\n## Stage 9: something else\n";
    const r = run("/runplan stage 1", [...planMsg(PLAN), ...planMsg(recap)]);
    expect(r.success).toBe(false);
    // It ran against the recap, not the real plan — and now says so.
    expect(r.response).toContain("It has stages: 9");
  });

  it("names the source when running the whole plan too", () => {
    const r = run("/runplan", planMsg(PLAN));
    expect(r.success).toBe(true);
    expect(r.response).toContain("Plan source: this session");
  });
});
